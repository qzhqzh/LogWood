...
8. **证据不可静默丢失**：存在正式 Evaluation 的对象不能直接物理删除，必须先显式归档或迁移评测。
...
空的可选字段必须省略，不能在 Prisma JSON 中写入 `undefined`。证据链接后续需要增加失效检查和内容快照策略。
...
草稿不执行完整发布门禁，便于先创建标题和评测对象，再逐步记录实验。API 和管理表单不得对草稿强制任务、结论、证据或完整评分。
...
首期不提供 Evaluation 物理删除接口。错误或过期内容通过 `archived` 保留审计和历史。

Target、Skill、Candidate 与 Evaluation 的外键使用 `onDelete=Restrict`；对应删除 API 会先检查 Evaluation 并在存在正式证据时返回 `409 ERR_SUBJECT_HAS_EVALUATIONS`。管理员必须先决定：

1. 保留对象并将其归档；
2. 将 Evaluation 显式迁移到后继对象；
3. 在未来有完整审计和来源快照机制后，再执行受控删除。

数据库约束是最后一道保护，不能仅依赖 UI 隐藏删除按钮。
