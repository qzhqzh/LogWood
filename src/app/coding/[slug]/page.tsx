...
export async function generateMetadata({ params }: { params: { slug: string } }): Promise<Metadata> {
  const target = await prisma.target.findFirst({
    where: { slug: params.slug, type: 'coding' },
  ...
  const description = target.description
    ...
    : `${target.name} AI Coding 工具评测 — 查看真实用户评分与使用体验`
  return {
    title: `${target.name} - AI Coding 评测`,
    description,
    alternates: { canonical: `${BASE_URL}/coding/${target.slug}` },
    openGraph: {
      title: `${target.name} - AI Coding 评测 | LogWood`,
      description,
      url: `${BASE_URL}/coding/${target.slug}`,
    },
  }
}
...
export default async function CodingDetailPage({ params }: TargetPageProps) {
  ...
  const jsonLd = {
    ...
    name: target.name,
    description: target.description || `${target.name} AI Coding 工具评测`,
