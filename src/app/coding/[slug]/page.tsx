...
export async function generateMetadata({ params }: { params: { slug: string } }): Promise<Metadata> {
  ...
  const description = target.description
    ...
    : `${target.name} 编码工具的真实使用记录、稳定性与工作流评测`
  return buildMetadata({
    title: `${target.name} - 编码工具资源`,
    description,
    path: `/coding/${target.slug}`,
  })
}
...
export default async function CodingDetailPage({ params }: TargetPageProps) {
  ...
  const path = `/coding/${target.slug}`
  const jsonLd = buildSoftwareApplicationJsonLd({
    name: target.name,
    description: target.description ?? `${target.name} 编码工具资源`,
    url: path,
        ...
        <div className="cyber-card rounded-3xl p-8 mb-8" style={{ borderColor: 'rgba(191, 0, 255, 0.2)' }}>
          <div className="flex items-start gap-6">
            ...
            <div className="flex-1">
              <p className="text-xs tracking-[0.2em] text-purple-300 uppercase mb-2">CODING RESOURCE</p>
