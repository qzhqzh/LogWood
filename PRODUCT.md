...
主要用户是长期整理 AI 提示词、工作流、实验、思想和实践经验的个人创作者与开发者。访客可以查找、复制、比较和阅读公开记录；作者通过收集箱、思想工作台和管理页维护讨论、草稿、来源、归属与发布状态。
...
作者的思想写作流复用同一套内容生命周期。当前 AI 聊天框是主要创作入口，`/forge` 是需要站内保存逐轮讨论时的备用入口：

```text
当前 AI 聊天（默认，不逐轮入库）
  或私有思想讨论（Candidate，站内备用）
  → AI 追问 / 只记录
  → Article draft 或当前文章的新版本
  → 作者检查并确认当前版本
  → Published Article
```

默认情况下，作者与 AI 在当前聊天中完成讨论和改稿；完整审阅稿得到明确确认后，Agent 才通过 MCP 创建文章草稿并发表精确版本。LogWood 不要求作者重复输入讨论过程。公众号版本是已确认文章的派生稿，默认由 AI 在聊天中生成、作者自行发布。
...
| 思想工作台 | `/forge` | 持续讨论并自动保存私有记录，幂等地生成或更新文章草稿；原结构化整理工具保留为次级入口 |
...
14. 思想讨论以私有 Candidate 保存；模型故障不回滚已经写入的作者消息，同一请求重试不会重复记录。
15. 思想工作台的“确认发表”同时批准并公开用户正在查看的版本；服务端必须校验版本号，任何后续正文变化都会重新回到草稿。
16. 聊天到文章的 Agent 流程不得自动保存逐轮对话；只有作者明确确认完整审阅稿后，才能先创建草稿，再以文章 ID、当前版本和固定确认令牌公开。公众号派生稿默认不外发。
